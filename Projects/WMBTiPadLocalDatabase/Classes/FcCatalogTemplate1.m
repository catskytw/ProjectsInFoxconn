...
-(void)loadProductData{
    ...
    [queryPattern prepareDicWithDictionary:(NSDictionary*)[QuerySQLProcess queryProductList:sCategoryKey withBrandId:brandId withColor:colorId withPrice:priceId withType:typeString]];
	//共有幾筆data
...
-(void)loadFilterCondition{
    ...
    //抓brand filter
    [filterQuery prepareDicWithDictionary:[QuerySQLProcess queryBrand:sKey]];
    ...
    [filterQuery prepareDicWithDictionary:[QuerySQLProcess queryColor:sKey]];
    ...
    //抓price filter
    [filterQuery prepareDicWithDictionary:[QuerySQLProcess queryPrice:sKey]];
...
-(void)colorSelected:(NSString*)color{
    ...
    for(int i=0;i<[keyArray count];i++){
        ...
        if([value isEqualToString:color]){
            ...
            [queryAttributeDic setValue:[keyArray objectAtIndex:i] forKey:@"productColorId"];
