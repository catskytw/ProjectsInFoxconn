...
- (id) initWithFrame:(CGRect)theFrame cancelEnabled:(BOOL)isCancelEnabled oneDMode:(BOOL)isOneDModeEnabled {
  if( self = [super initWithFrame:theFrame] ) {
    ...
    if (isCancelEnabled) {
      ...
      imageView=[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"img_qr_center.png"]];
      [imageView setFrame:CGRectMake(87, 69, 147, 147)];
      [self addSubview:imageView];
      [imageView release];
      
      UIView *_blackView=[UIView new];
      [_blackView setBackgroundColor:[UIColor colorWithRed:0 green:0 blue:0 alpha:0.5]];
      [_blackView setFrame:CGRectMake(0, 305, 320, 63)];
      [self addSubview:_blackView];
      [_blackView release];
      
      UIImageView *qrcodeIcon=[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"img_qrcode.png"]];
      [self addSubview:qrcodeIcon];
      qrcodeIcon.center=CGPointMake(100, 335);
      [qrcodeIcon release];
      
      UILabel *qrLabel=[[UILabel alloc]initWithFrame:CGRectMake(0, 0, 200, 63)];
      [qrLabel setFont:[UIFont fontWithName:@"Helvetica" size:22.0]];
      [qrLabel setTextColor:[UIColor whiteColor]];
      [qrLabel setText:@"請掃瞄QR Code"];
      [qrLabel setBackgroundColor:[UIColor clearColor]];
      [self addSubview:qrLabel];
      qrLabel.center=CGPointMake(230, 335);
      [qrLabel release];
      */
  }
  return self;
