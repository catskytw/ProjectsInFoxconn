...
//花協專案,bottomPanel要mark
#ifdef SHOWBOTTOMPANEL
...
-(void)viewWillAppear:(BOOL)animated{
	...
	MapFeature *nineFeature=[dataCtrl getFeatureObject:nowPosition withFeatureType:FeatureTypePoint];
	CGPoint point=[[nineFeature.points objectAtIndex:0]CGPointValue];
	//若為全區地圖,應該全區繪製出來	
...
//產生shortestPath的路線經過圖
-(void)productShortestPathPoint:(NSInteger)targetPosition{
	[shortestPathEdgeArray removeAllObjects];
	
	AStar *a=[[AStar alloc]initWithStartpoint:[NSString stringWithFormat:@"%i",nowPosition] withEndpoint:[NSString stringWithFormat:@"%i",targetPosition]];
...
-(IBAction)buttonAction:(id)sender{
	...
	if(!isRunningMap){
		if(nowPosition==destPosition){
			...
			[self productShortestPathPoint:destPosition];
			if(thisExhibitionInfoObject!=nil){
...
-(void)endWalking{
	[self stopWalking];
	//到目的地,要將前進之按鈕disable
	myTopPanel.forwardBtn.enabled=NO;
	if(displayLink!=nil){
		[displayLink invalidate];
		displayLink=nil;
	}
...
-(void)keepWalking{
	...
	if(eachPoint.featureId!=-1 ){
		...
		MapFeature *thisFeature=[ctrl getFeatureObject:eachPoint.featureId withFeatureType:FeatureTypePoint];
