...
-(id)initWithFileName:(NSString*)thisFileName{
	if((self=[super init])){
		...
		if (path != nil){
			...
			featureCount=[ToolsFunction getIntFromLittleNSData:[totalData subdataWithRange:NSMakeRange(0, 4)]];
		}
	}
	return self;
}
-(MapFeature*)getFeatureObject:(NSInteger)indexInData withFeatureType:(NSInteger)featureType{
	if(indexInData>featureCount){
		NSLog(@"ERROR! Big featureId:%i, fileName:%@",indexInData,path);
		return nil;
	}
	
	NSData *featureNSData=[self getFeatureData:indexInData];
	//整個feature的長度
	int style=[ToolsFunction getIntFromLittleNSData:[featureNSData subdataWithRange:NSMakeRange(0, 2)]];
	...
	switch (style) {
		case FeatureTypePoint:
			styleLength=10;
			nameLength=[ToolsFunction getIntFromLittleNSData:[featureNSData subdataWithRange:NSMakeRange(styleLength, 2)]];
			pointNum=[ToolsFunction getIntFromLittleNSData:[featureNSData subdataWithRange:NSMakeRange(styleLength+2+nameLength, 2)]];
...
-(NSMutableArray*)getPoints:(NSInteger)startPosition withFeatureData:(NSData*)featureData withPointNum:(NSInteger)pointNum{
	...
	for(int i=0;i<pointNum;i++){
		int x=[ToolsFunction getIntFromLittleNSData:[featureData subdataWithRange:NSMakeRange(startPosition+8*i, 4)]];
		int y=[ToolsFunction getIntFromLittleNSData:[featureData subdataWithRange:NSMakeRange(startPosition+4+8*i, 4)]];
...
-(NSData*)getFeatureData:(NSInteger)indexInData{
	...
	int featureStartPosition=[ToolsFunction getIntFromLittleNSData:[indexNode subdataWithRange:NSMakeRange(0, 4)]];
	int featureLength=[ToolsFunction getIntFromLittleNSData:[indexNode subdataWithRange:NSMakeRange(4, 2)]];
