...
+(void)delWaitCursor{
	[[(((OveExpoAppDelegate*)[[UIApplication sharedApplication] delegate]).tabBarController).view viewWithTag:1000]removeFromSuperview];	
	[waitView release];
	waitView=nil;
...
+(NSString*)floorName:(NSInteger)floorType{
    ...
    switch (floorType) {
        case A_2F:
            floorString=AMLocalizedString(@"A_floor2", nil);
            break;
        case A_3F:
            floorString=AMLocalizedString(@"A_floor3", nil);
            break;
        case B_1F:
            floorString=AMLocalizedString(@"B_floor1", nil);
            break;
        case B_2F:
            floorString=AMLocalizedString(@"B_floor2", nil);
            break;
    }
    return floorString;
}
