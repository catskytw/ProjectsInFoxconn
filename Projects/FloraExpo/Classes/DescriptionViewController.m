...
-(UIWebView*)getWebView:(NSString*)stringText{
	...
	if([stringText isEqualToString:@""] || stringText==nil){
		thisWebView=[[UIWebView alloc]initWithFrame:CGRectMake(0, 0, 0, 0)];
	}else{
		thisWebView=[[UIWebView alloc]initWithFrame:CGRectMake(0, 0, actualSize.width, actualSize.height)];
	}
	//UIWebView *thisWebView=[UIWebView new];
...
-(void)constructThisPage{
	...
	for(DescriptionObject *thisObject in dataStringArray){
		...
		}else if(thisObject.descriptionType==DescriptionTypeWebView){
			[uiArray addObject:[self getWebView:thisObject.textString]];
		}
	}
	for(UIView *element in uiArray){
		UILabel *thisLabel;
		UILabel *thisView;
		UIWebView *thisWebView;
		switch (element.tag) {
			case DescriptionTypeTitle:
				thisLabel=(UILabel*)element;
				...
				[tmpLabel release];
				break;
			case DescriptionTypeContent:
				thisView=(UILabel*)element;
				[thisView setFrame:CGRectMake(15, startY, thisView.frame.size.width, thisView.frame.size.height)];
				break;
			case DescriptionTypeWebView:
				thisWebView=(UIWebView*)element;
				
				//[thisWebView setFrame:CGRectMake(15, startY, thisWebView.frame.size.width, thisWebView.frame.size.height)];
				[thisWebView setFrame:CGRectMake(15, startY, thisWebView.frame.size.width, thisWebView.frame.size.height)];
				thisWebView.backgroundColor=[UIColor clearColor];
				break;
		}
