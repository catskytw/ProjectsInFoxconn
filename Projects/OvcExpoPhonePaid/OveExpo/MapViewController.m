...
#endif
    NSLog(@"targetPosition %d",targetPosition);
...
-(void)makeRouting:(NSNotification*)noti{
    ...
    [self drawStartAndTargetIcon:targetPosition withNowPosition:nowPosition];
    if(isDifferentFloor){
        NSString *message=[NSString stringWithFormat:AMLocalizedString(@"differentFloor", nil),[LocationInfoObject nowPosition].name,[LocationInfoObject targetPosition].name];
        UIAlertView *alert=[[UIAlertView alloc] initWithTitle:AMLocalizedString(@"login_err_alert_title", nil) message:message delegate:nil cancelButtonTitle:AMLocalizedString(@"close", nil) otherButtonTitles: nil];
        [alert show];
    }
}
