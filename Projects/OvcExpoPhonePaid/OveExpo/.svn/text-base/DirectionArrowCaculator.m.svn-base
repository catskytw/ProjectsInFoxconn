...
-(NSDictionary*)searchFeaturePointInArea:(CGRect)searchingRect withStandingPoint:(CGPoint)basePoint{
	...
	MapDataCtrl *pointData=[FeatureCtrlCollections current].locationDataCtrl;
	//總共有幾個feature
	...
	for(int i=0;i<totalPointFeature;i++){
        ...
        MapPoint *thisPoint=(MapPoint*)thisFeature.styleObject;
        if(thisPoint.bmpId==0 || thisPoint.bmpId==9999)
            continue;
		//Coordination *featurePoint=(Coordination*)[thisFeature.drawingArray objectAtIndex:0];
        CGPoint _point=[(NSValue*)[thisFeature.points objectAtIndex:0] CGPointValue];
		//if(CGRectContainsPoint(searchingRect, CGPointMake(featurePoint.x,featurePoint.y))){
        if(CGRectContainsPoint(searchingRect, CGPointMake(_point.x,_point.y))){
			double thisFeatureAngle=[self caculateAngle:basePoint withPurposePoint:CGPointMake(_point.x,_point.y)];
			//正北方是345~15度,因此需校正
