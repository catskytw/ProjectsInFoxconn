...
- (void)zxingController:(ZXingWidgetController*)controller didScanResult:(NSString *)_result{
    ...
    @try {
        ...
        if ([[sepStrings objectAtIndex:0] intValue]==4 && [QRCodeSingletonObject current].functionType!=4) {
            UIAlertView *alert=[[UIAlertView alloc] initWithTitle:AMLocalizedString(@"", nil) message:AMLocalizedString(@"", nil) delegate:nil cancelButtonTitle:AMLocalizedString(@"close", nil) otherButtonTitles:nil];
            [alert show];
            return;
        }
