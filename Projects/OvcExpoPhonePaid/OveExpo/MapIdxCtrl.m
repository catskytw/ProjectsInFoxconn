...
-(id)initWithFileName:(NSString*)thisFileName{
	if((self=[super init])){
        ...
        NSString *path=[NSString stringWithFormat:@"%@.idx",thisFileName];
		if (path != nil){
			...
			//連續讀header
			startX=[MiscTool getIntFromLittleNSData:[totalData subdataWithRange:NSMakeRange(0, 4)]];
			startY=[MiscTool getIntFromLittleNSData:[totalData subdataWithRange:NSMakeRange(4, 4)]];
			cellWidth=[MiscTool getIntFromLittleNSData:[totalData subdataWithRange:NSMakeRange(8, 4)]];
			cellHeight=[MiscTool getIntFromLittleNSData:[totalData subdataWithRange:NSMakeRange(12, 4)]];
			cellColumn=[MiscTool getIntFromLittleNSData:[totalData subdataWithRange:NSMakeRange(16, 4)]];
			cellRow=[MiscTool getIntFromLittleNSData:[totalData subdataWithRange:NSMakeRange(20, 4)]];
...
-(void)getAllFeatureIdInCollection:(NSData*)indexNodeData withDictionary:(NSMutableDictionary*)inputDictionary{
	int startPosition=[MiscTool getIntFromLittleNSData:[indexNodeData subdataWithRange:NSMakeRange(0, 4)]];
	int num=[MiscTool getIntFromLittleNSData:[indexNodeData subdataWithRange:NSMakeRange(4, 4)]];
	for(int i=0;i<num;i++){
		FeatureIdxObject *fio=[[[FeatureIdxObject alloc]init]autorelease];
		fio.styleType=[MiscTool getIntFromLittleNSData:[totalData subdataWithRange:NSMakeRange(startPosition+i*12, 4)]];
		fio.positionInDatFile=[MiscTool getIntFromLittleNSData:[totalData subdataWithRange:NSMakeRange(startPosition+4+i*12, 4)]];
		fio.featureId=[MiscTool getIntFromLittleNSData:[totalData subdataWithRange:NSMakeRange(startPosition+8+i*12, 4)]];
		
		//NSLog(@"%i",fio.featureId);
