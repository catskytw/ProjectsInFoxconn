...
ts_time_decompose(const unsigned long time_in_sec, const char padded_char)
{
    ...
    if (hour == 0)
        printf("   %2u:%02u%c", min, sec, padded_char);
    else if (hour < 100)
        printf("%2lu:%02u:%02u%c", hour, min, sec, padded_char);
    else
        printf("%6lu h%c", hour, padded_char);
}
...
timestatus(const lame_global_flags * const gfp)
{
    ...
    printf("\r%6i/%-6i", frameNum, totalframes);
    printf(percent < 100 ? " (%2d%%)|" : "(%3.3d%%)|", percent);
    ...
    printf(proc_time.speed_index <= 1. ?
...
/*    printf ("%s", Console_IO.str_clreoln ); */
