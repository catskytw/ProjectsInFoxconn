...
-(BOOL)MouseDown:(CGPoint)point{
	...
	if (callback.m_fixture)
	{
        ...
        //explosion
        [self lunchBomb:CGPointMake(p.x, p.y) withForce:30 withRadius:30];
    }
...
-(void)lunchBomb:(CGPoint)point withForce:(NSInteger)force withRadius:(CGFloat)radius{
    b2Vec2 touchPoint=b2Vec2(point.x,point.y);
    
    //every body has the impulse? really?
    //further more: maybe using AABBQuery is better.
    for (b2Body *b=world->GetBodyList(); b; b=b->GetNext()) {
        b2Vec2 bodyPoint=b2Vec2(b->GetPosition().x,b->GetPosition().y);
        
        CGFloat distance=b2Distance(touchPoint, bodyPoint);
        if(distance > radius) return;
        CGFloat strength=(radius-distance)/radius;
        CGFloat actualForce=force*strength;
        CGFloat angle=atan2f(bodyPoint.y-touchPoint.y, bodyPoint.x-touchPoint.x);
        b->ApplyLinearImpulse(b2Vec2(cosf(angle)*actualForce,sinf(angle)*actualForce),b->GetPosition());
    }
}
@end
