...
int CDECL is_mpeg_file_format(int input_format);
FILE CDECL *init_outfile(char *outPath, int decode);
void CDECL init_infile(lame_global_flags *, char *inPath, int *enc_delay, int *enc_padding);
void CDECL close_infile(void);
int CDECL get_audio(lame_global_flags * const gfp, int buffer[2][1152]);
int CDECL get_audio16(lame_global_flags * const gfp, short buffer[2][1152]);
int CDECL WriteWaveHeader(FILE * const fp, const int pcmbytes,
...
*/
int CDECL lame_decoder(lame_global_flags * gfp, FILE * outf, int skip, char *inPath, char *outPath,
                     int *enc_delay, int *enc_padding);



void CDECL SwapBytesInWords(short *loc, int words);
